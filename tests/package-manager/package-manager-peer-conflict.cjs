...
try {
  ...
  fs.writeFileSync(
    path.join(lockedPeerRoot, "hutch.lock"),
    `${JSON.stringify(
      {
        lockfileVersion: 1,
        configVersion: 1,
  ...
  fs.writeFileSync(
    path.join(legacyTarballRoot, "hutch.lock"),
    `${JSON.stringify(
      {
        lockfileVersion: 1,
        workspaces: {
  ...
  const savedLegacyLock = fs.readFileSync(path.join(legacyTarballRoot, "hutch.lock"), "utf8");
