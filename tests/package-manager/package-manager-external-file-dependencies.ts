...
try {
  ...
  // nested must be resolvable from external under Node resolution; the exact
  // placement (nested vs hoisted) is not part of Hutch's contract.
  assert.ok(
    fs.existsSync(path.join(installedExternal, "node_modules", "nested", "package.json")) ||
      fs.existsSync(path.join(app, "node_modules", "nested", "package.json")),
  );
  assert.ok(!fs.existsSync(path.join(external, "node_modules")));

  console.log("package manager external file dependencies passed");
} finally {
  if (!process.env.COTTONTAIL_KEEP_TEST_TEMP) fs.rmSync(testRoot, { recursive: true, force: true });
}
