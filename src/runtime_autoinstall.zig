...
pub fn prepare(
    ...
    try install_environment.put("HUTCH_INSTALL_CACHE_DIR", download_cache);
...
pub fn autoInstallCacheRoot(
    ...
    if (init.environ_map.get("HUTCH_INSTALL_CACHE_DIR") orelse init.environ_map.get("BUN_INSTALL_CACHE_DIR")) |configured| {
    ...
    const dash_home = if (init.environ_map.get("HUTCH_HOME")) |home|
        try allocator.dupe(u8, home)
    else if (init.environ_map.get("DASH_HOME")) |home|
        try allocator.dupe(u8, home)
    else if (init.environ_map.get("HOME")) |home|
        try std.fs.path.join(allocator, &.{ home, ".hutch" })
