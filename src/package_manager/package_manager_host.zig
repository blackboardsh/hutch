...
pub fn scanDependencies(
    ...
    try argv.appendSlice(allocator, &.{
        runtime,
        "--cottontail-scan-package-imports",
        working_dir,
    });
    try argv.appendSlice(allocator, entry_points);
    ...
    if (childExitCode(result.term) != 0) {
        ...
        return error.PackageImportScanFailed;
    }
    ...
    const parsed = std.json.parseFromSlice(
        ...
        try stderr.writeAll("hutch: invalid Cottontail package import scan response");
        ...
        return error.InvalidPackageImportScan;
    };
    defer parsed.deinit();

    const dependencies = try allocator.alloc([]const u8, parsed.value.len);
    for (parsed.value, dependencies) |dependency, *owned| {
        owned.* = try allocator.dupe(u8, dependency);
    }
    return dependencies;
}
