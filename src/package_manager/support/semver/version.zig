        ...
        pub const Tag = extern struct {
            ...
            pub fn cloneInto(this: Tag, slice: []const u8, buf: *[]u8) Tag {
                ...
                if (this.pre.isInline()) {
                    ...
                    @memcpy(buf.*[0..pre_slice.len], pre_slice);
                ...
                if (this.build.isInline()) {
                    ...
                    @memcpy(buf.*[0..build_slice.len], build_slice);
                ...
                switch (input[i]) {
                    ...
                    'x', '*', 'X' => {
                        ...
                        if (result.wildcard == .none) {
                            switch (part_i) {
                                0 => {
                                    result.wildcard = .major;
                                    part_i = 1;
                                },
                                1 => {
                                    result.wildcard = .minor;
                                    part_i = 2;
                                },
                                2 => {
                                    result.wildcard = .patch;
                                    part_i = 3;
                                },
                                else => {},
                            }
...
const Output = support.Output;
const assert = std.debug.assert;
const strings = support.strings;

const string_types = @import("string.zig");
const ExternalString = string_types.ExternalString;
const SlicedString = string_types.SlicedString;
const String = string_types.String;
const Wildcard = @import("common.zig").Wildcard;
