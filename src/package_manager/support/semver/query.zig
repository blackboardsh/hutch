...
pub const Group = struct {
    ...
    pub fn jsonStringify(this: *const Group, writer: anytype) !void {
        const temp = try std.fmt.allocPrint(this.allocator, "{f}", .{this.fmt(this.input)});
        defer this.allocator.free(temp);
    ...
    pub fn deinit(this: *const Group) void {
        ...
        var query_node = this.head.head.next;
        while (query_node) |node| {
            const next = node.next;
            allocator.destroy(node);
            query_node = next;
        }

        var list_node = this.head.next;
        while (list_node) |node| {
            const next = node.next;
            query_node = node.head.next;
            while (query_node) |query| {
                const next_query = query.next;
                allocator.destroy(query);
                query_node = next_query;
            }
            allocator.destroy(node);
            list_node = next;
        }
...
pub const Token = struct {
    ...
    pub const Wildcard = @import("common.zig").Wildcard;
};
...
const strings = support.strings;

const Range = @import("range.zig");
const SlicedString = @import("string.zig").SlicedString;
