...
fn commandFromString(command: []const u8) ?Command {
    // v1 of the built-in resolver installs only. Everything else belongs to
    // an explicitly selected external package manager. "add" stays routable
    // for runtime auto-install's internal staging installs; Hutch's public
    // CLI never dispatches it.
...
fn parseOptions(allocator: std.mem.Allocator, args: []const [:0]const u8) !Options {
    ...
    while (index < args.len) : (index += 1) {
            ...
            (std.mem.eql(u8, arg, "-a") and (options.command == .add or options.command == .install)))
        {
        ...
        } else if (std.mem.eql(u8, arg, "--access") or std.mem.startsWith(u8, arg, "--access=")) {
            ...
            options.registry_auth_option_used = true;
        } else if (std.mem.eql(u8, arg, "--auth-type") or std.mem.startsWith(u8, arg, "--auth-type=")) {
            return error.InvalidPackageManagerOption;
...
const Manager = struct {
    ...
    fn execute(manager: *Manager) !u8 {
        ...
        if (!manager.options.silent and manager.blocked_scripts > 0) {
            try manager.stdout.print(
                "\nSkipped {d} lifecycle script{s}: Hutch never runs install scripts. Select an external packageManager if a package requires them.\n",
                .{ manager.blocked_scripts, if (manager.blocked_scripts == 1) "" else "s" },
            );
        }
        try manager.stdout.flush();
    ...
    fn loadLockfile(manager: *Manager, root: *const Value) !void {
        ...
        const hutch_text_path = try std.fs.path.join(manager.allocator, &.{ manager.root_dir, Lockfile.file_name });
        const source = std.Io.Dir.cwd().readFileAlloc(
            manager.init_data.io,
            hutch_text_path,
            manager.allocator,
    ...
    fn queuePackageScripts(
        ...
        // Hutch's resolver never runs lifecycle scripts; it only counts the
        // scripts it skipped so the install summary can say so. Packages that
        // require install scripts belong to an explicit external manager.
        _ = alias;
        _ = package_name;
        _ = version_value;
        _ = direct;
    ...
    fn writeTextLockfile(manager: *Manager, root: *Value, save_bun_lockfile: bool) !void {
        ...
        if (manager.init_data.environ_map.get("COTTONTAIL_PM_ERROR_TRACE") != null) {
            try manager.stderr.writeAll("--- generated hutch.lock ---\n");
            try manager.stderr.writeAll(output.written());
            try manager.stderr.writeAll("--- end generated hutch.lock ---\n");
        }

        if (save_bun_lockfile) {
            // hutch.lock is always the text format and Hutch's own emission is
            // canonical; no runtime service round-trip. Migration sources
            // (bun.lock, package-lock.json, ...) are left untouched.
            const hutch_path = try std.fs.path.join(manager.allocator, &.{ manager.root_dir, Lockfile.file_name });
            try std.Io.Dir.cwd().writeFile(manager.init_data.io, .{ .sub_path = hutch_path, .data = output.written() });
            if (!manager.options.silent and manager.options.command != .link) try manager.stderr.writeAll("Saved hutch.lock\n");
        }
    }
            ...
            if (package_json.* == .object) {
                if (package_json.object.get("scripts")) |scripts| {
                    if (scripts == .object) {
                        ...
                        inline for (Lockfile.lifecycle_script_names) |script_name| {
...
fn packageCachePath(init: std.process.Init, allocator: std.mem.Allocator) ![]const u8 {
    if (init.environ_map.get("HUTCH_INSTALL_CACHE_DIR")) |cache| return allocator.dupe(u8, cache);
    ...
    // The package cache lives in Hutch's home, beside its other stores.
    if (init.environ_map.get("HUTCH_HOME")) |home| return std.fs.path.join(allocator, &.{ home, "cache", "npm" });
    if (init.environ_map.get("DASH_HOME")) |home| return std.fs.path.join(allocator, &.{ home, "cache", "npm" });
    if (init.environ_map.get("HOME")) |home| return std.fs.path.join(allocator, &.{ home, ".hutch", "cache", "npm" });
    if (init.environ_map.get("USERPROFILE")) |home| return std.fs.path.join(allocator, &.{ home, ".hutch", "cache", "npm" });
